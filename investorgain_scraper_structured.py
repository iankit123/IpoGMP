...
class InvestorGainScraperStructured:
    """Structured InvestorGain scraper using data-label attributes"""
    ...
    def scrape_ipo_data(self):
        """Scrape real IPO data from InvestorGain website using structured HTML"""
        logger.info("Starting structured InvestorGain IPO data scraping...")
        ...
        for url in self.base_urls:
            try:
                ...
                ipo_data = self._extract_structured_ipo_data(soup)
                if ipo_data:
    ...
    def _extract_structured_ipo_data(self, soup):
        """Extract IPO data using data-label attributes"""
        ipo_data = []
        
        # Look for table rows that contain IPO data
        # Based on the HTML structure, we need to find rows with data-label attributes
        ...
        try:
            ...
            for cell in cells_with_labels:
                ...
                if data_label == 'name' or data_label == 'ipo name':
                    ...
                    ipo_info['lot_size'] = self._parse_number(cell_text)
...
def scrape_investorgain_structured():
    """Main function to scrape InvestorGain data using structured HTML"""
    scraper = InvestorGainScraperStructured()
...
if __name__ == "__main__":
    scrape_investorgain_structured()
