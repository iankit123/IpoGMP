...
if (window.ipoTrackerInitialized) {
    ...
    async function loadIPOData() {
        try {
            ...
            // Merge data strategies:
            // 1. Create a map of InvestorGain data by name
            const investorgainMap = new Map()
            if (investorgainResult.data) {
                investorgainResult.data.forEach(ipo => {
                    // Normalize name for matching
                    const normalizedName = ipo.name.toLowerCase().trim()
                    investorgainMap.set(normalizedName, ipo)
                })
            }

            // 2. Process primary IPOs and enrich with InvestorGain data
            const primaryIPOs = iposResult.data || []
            const mergedIPOs = primaryIPOs.map(ipo => {
                const normalizedName = ipo.name.toLowerCase().trim()
                const investorgainData = investorgainMap.get(normalizedName)

                if (investorgainData) {
                    // Enrich with Price and Size if missing in primary
                    return {
                        ...ipo,
                        price: ipo.price || investorgainData.price,
                        ipo_size: ipo.ipo_size || investorgainData.ipo_size,
                        gmp_value: ipo.gmp_value || investorgainData.gmp_value,
                        gmp_percentage: ipo.gmp_percentage || investorgainData.gmp_percentage
                    }
                }
                return ipo
            })

            // 3. Add any InvestorGain IPOs that aren't in primary list
            if (investorgainResult.data) {
                investorgainResult.data.forEach(ipo => {
                    const normalizedName = ipo.name.toLowerCase().trim()
                    // Check if we already have this IPO (by name matching)
                    const exists = primaryIPOs.some(p => p.name.toLowerCase().trim() === normalizedName)
                    if (!exists) {
                        mergedIPOs.push(ipo)
                    }
                })
            ...
            allIPOs = mergedIPOs
            allInvestorGainIPOs = investorgainResult.data || [] // Keep raw for reference
            ...
            console.log(`📊 Merged Data: ${filteredIPOs.length} IPOs`)
